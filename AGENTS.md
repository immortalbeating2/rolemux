...
RoleMux 的目标是建设一个轻量多 CLI 工作流插件/工具包，让当前 AI CLI 能按角色调用 `codex`、`claude`、`agy`、`grok`、`opencode` 等其他 AI CLI 协作完成分析、规划、实现、审查和验证任务。
...
- provider adapter 不得散落在 Skill 文档或命令实现里；`codex`、`claude`、`agy`、`grok`、`opencode` 的真实调用必须集中在 adapter 层。
...
- 新增或修改 provider adapter，包括 `codex`、`claude`、`agy`、`grok`、`opencode` 的调用参数。
...
3. M2：Provider MVP，完成 Codex、Claude、Agy、Grok Build、OpenCode 五个 adapter 的真实调用。阶段文档：`spec/phases/m2-provider-adapters.md`；实施文档：`spec/implementation/m2-provider-adapters-plan.md`。
...
- `rolemux doctor` 能检查 `codex`、`claude`、`agy`、`grok`、`opencode`。
- `rolemux run` 能按 provider + role 执行任务或 dry-run。
- 至少支持 Codex、Claude、Antigravity/agy、Grok Build、OpenCode 五类 provider adapter。
...
- 文件名使用 kebab-case；类型使用 PascalCase；函数、变量和字段使用 camelCase；provider 值使用 `codex`、`claude`、`agy`、`grok`、`opencode`。
