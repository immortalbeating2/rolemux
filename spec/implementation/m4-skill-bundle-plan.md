...
**Goal:** 实现通用 RoleMux Skill、默认 roles 和按目标安装复制逻辑。

**Architecture:** 通用 Skill 文件只描述触发条件和 RoleMux CLI 调用；role prompt 独立存放；install 命令负责从通用源复制到 Codex/Claude/plugin 目标并保护用户已有文件。
...
- Create: `skills/rolemux-workflow/SKILL.md`
...
- [ ] 编写通用 Skill，包含触发条件、调用命令、产物读取和禁止事项。
- [ ] 验证 install 会把通用 Skill 源复制到 Codex/Claude 目标目录。
